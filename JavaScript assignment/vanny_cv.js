...
let hoverEventt = () => {
   ...
   for (var k=0; k<alpha.length; k++) {
      alpha[k].style.color="rgb(223, 32, 191)";
   }
}
...
let celements2 = document.querySelectorAll('.centered-2'); //creating a nodelist of the elements with CSS class .centered-2

let nohoverEventt = () => {
   var beta = document.querySelectorAll('.centered-2');
   for (var j=0; j<beta.length; j++) {
      beta[j].style.color="black";
   }
}
celements2.forEach((item) => {
   item.addEventListener('mouseout', nohoverEventt)       //event on Mouseout for each item
});
...
let nohoverEvent = () => {                               //creating the 'event' to be passed on each item in the node
   ...
   for (var i=0; i<y.length; i++) {                      //for loop that goes through all elements with class .centered
      y[i].style.color="black";                          //for each element, this event will happen on Mouseout
   }
}
